...
// post, the post's comment count and rank move with it, and the author's identity and handle last
// at least as long. Nothing is written to a post that has gone or expired.
//
// KEYS[1] the post, KEYS[2] the comment, KEYS[3] the author's identity, KEYS[4] their handle
// ARGV[1] now in milliseconds, ARGV[2] the rank's score weight, then the comment's field/value pairs
