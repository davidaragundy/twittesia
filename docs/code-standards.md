...
**CS-5.** `shared/` carries no domain. If a thing knows what a `Post` is, it
...
it in the file name — `ghost-form.tsx`, not `ghost/form.tsx`.
_Review._
...
repeats or skips the write, and the breakage does not look like a rendering bug
when you find it. _Review._
...
`@/features`, anything else under `@/`, stylesheets. Stylesheets sort last,
whether they are side-effect imports or CSS modules. **Tooling** — it autofixes,
so you will never see it fail.
