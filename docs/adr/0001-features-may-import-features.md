...
the dependency it appears to remove — moving `Post` into `shared/` because
`comments` needs it leaves the same dependency in place, now without a name, and
...
**A `shared/` organised by domain** (`shared/posts/`, `shared/chats/`).
...
`auth`, importable by anyone. Rejected once feature-to-feature imports were
allowed: the layer exists only to carve out an exception to a rule that no
longer exists.
...
usual candidate — a profile page showing a follow button — never becomes an
import, because the route assembles a component from `profiles` and a component
from `follows` without either feature knowing about the other. What remains are
data and type dependencies, which run in one direction naturally.
