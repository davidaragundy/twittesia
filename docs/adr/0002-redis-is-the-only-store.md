...
- **Keys expire with what they belong to.** A post's hash and the record of who
  reacted to it expire when the post does; a comment expires with its post.
  Nothing is deleted by a schedule except files in Blob, since an expiring key
  notifies nobody: every file's deletion time sits in a sorted set, and a sweep
  deletes what is due.
