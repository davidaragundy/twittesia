...
`auth`, `settings`, `profiles`, `posts`, `comments`, `media`, `landing`, `explore`, `chat`, `moderation`.
Use `repo`, `ci` or `agents` for work that touches no feature.
...
```
type/twt-<issue-number>
```

The `twt-` prefix is always required, and the type must come from the table above.
...
```
fix(auth): clear validation error state on input change
...
```
Good:  [Bug]: posts past their expiry still appear in the feed
Bad:   fix(posts): posts past their expiry still appear in the feed
```
...
`.github/CODEOWNERS` requests review from `davidaragundy` automatically.

- **If someone other than `davidaragundy` opened the PR**, wait for `davidaragundy`'s approval. Only `davidaragundy` may merge without one.
- **If `davidaragundy` opened the PR**, GitHub does not request review from the author and self-approval is rejected, so no approving review is possible. `davidaragundy` merges it themselves. Nothing reviews these PRs — keep them small and describe them well, because that description is the only review they get.
...
| Hook         | What it does                                                 |
| ------------ | ------------------------------------------------------------ |
| `pre-commit` | Runs `lint-staged` — oxlint and oxfmt on staged files        |
